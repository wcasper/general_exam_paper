...
\subsection{Morita Equivalence and the Brauer Group}
...
\begin{defn}
Let $A$ be an $R$-order in $\Delta$.  We define the \vocab{reduced norm} $\nrd(a)$, \vocab{reduced trace} $\trd(a)$, and \vocab{reduced characteristic polynomial} $\chrd(x;a)$ as follows.  
...
\begin{defn}
The \vocab{ramification index} of $D$ over $K$ is the unique positive integer $e = e(D/K)$ such that $w(D) = e^{-1}\bbz\subseteq \bbz$.  The \vocab{inertia degree} of $D$ over $K$ is then defined to be $f = f(D/K) = \dim_K(D)/e$.  The \vocab{normalized valuation} of $D$ over $K$ is defined by $\nu_D(x) = w(x)/e$.
\end{defn}
...
\begin{thm}
The $R$-algebra $A$ is a finitely generated free $R$-module.  In fact, for $f = f(D/K)$ and $e = e(D/K)$, $A/J(A)$ has dimension $f$ as a $k$-vector space, and if $a_1,\dots, a_f\in  A$ satisfy $A/J(A) = \vspan_k\{\ol a_1,\dots \ol a_f\}$, then
$$\{a_i\pi_D^j:1\leq i\leq f,\ 0\leq j\leq e-1\}$$
is a free $R$-basis for $A$.
\end{thm}
\begin{proof}
...
\begin{defn}
Let $R$ be integrally closed, and $A$ be a maximal $R$-order in $\Delta$.  For any prime ideal $\p\in\spec(R)$, we say that $\Delta$ \vocab{ramifies} at $\p$ if $A_\p$ is not Azumaya.  We define the \vocab{ramification locus} of $\Delta$, denoted $\ram(\Delta)$, to be the set of all prime ideals $\p\in\spec(R)$ at which $\Delta$ ramifies, and the \vocab{Azumaya locus} of $\Delta$ to be the complement of the ramification locus.
\end{defn}
...
\section{Maximal Orders on Schemes}
\subsection{Basic Definitions and Facts}
Let $X$ be an integral scheme, with fraction field $K$ and generic point $\eta$, and let $\Delta$ be a central simple $K$-algebra.  As an abuse of notation, we will use $\Delta$ interchangeably with the constant sheaf on $X$ induced by $\Delta$.
\begin{defn}
A \vocab{$\sheaf O_X$-order} $A$ in $\Delta$ is a coherent $\sheaf O_X$-module which is a $\sheaf O_X$-subalgebra of the constant sheaf $\Delta$ and satisfies $A_\eta = \Delta$.  The set $\Ord_{X,\Delta}$ of all $\sheaf O_X$-orders in $\Delta$ is again a poset, and a maximal element is called a \vocab{maximal $\sheaf O_X$-order in $\Delta$}.
\end{defn}

We next state some basic properties of $\sheaf O_X$-orders.  The proofs are similar to the affine case and are therefore omitted for brevity.
\begin{prop}
Let $A$ be an $\sheaf O_X$-order in $\Delta$.  Then the following are equivalent
\begin{enumerate}[(a)]
\item  $A$ is maximal
\item  $A_x$ is a maximal $\sheaf O_{X,x}$-order in $\Delta$ for every $x\in X$
\end{enumerate}
If in addition $X$ is normal, then (a) or (b) is also equivalent to
\begin{enumerate}[(a)]
\setcounter{enumi}{2}
\item  $A$ is a reflexive $\sheaf O_X$-module and $A_x$ is maximal $\forall x\in X^1$
\end{enumerate}
where here $X^1$ denotes the set of codimension $1$ points of $X$
\end{prop}

\begin{prop}
Suppose that $X$ is normal and let $A$ be an $\sheaf O_X$-order in $\Delta$.  Then $A$ is contained in a maximal $\sheaf O_X$-order $B$ in $\Delta$.
\end{prop}

\begin{prop}
Suppose that $X$ is normal and that $A$ is an Azumaya $\sheaf O_X$-order in $\Delta$.  Then $A$ is maximal.
\end{prop}

\subsection{Ramification Locus}
In this section, let $X$ be a normal integral scheme.
\begin{prop}
Let $A$ be an $\sheaf O_X$-order in $\Delta$.  Then 
\end{prop}

\subsection{Maximal Orders in Quaternion Algebras on Surfaces}

\section{The Brauer Group of a Scheme}
\section{The Brauer Group of a Surface}

\section{Maximal Orders on a Surface}

\section{Construction of a Nonrational Unirational Variety}
